...
Enable `revokeSessionsOnPasswordReset` to invalidate all existing sessions when a password is reset:
...
To use a different algorithm (e.g., Argon2id), provide custom `hash` and `verify` functions:
