...
describe('AdminUsersController', () => {
  ...
  describe('listSessions', () => {
    it('calls listUserSessions with userId and platformRole', async () => {
      adminService.listUserSessions.mockResolvedValue([] as never);

      await controller.listSessions(baseSession, 'user-1');

      expect(adminService.listUserSessions).toHaveBeenCalledWith({
        userId: 'user-1', platformRole: 'admin', activeOrganizationId: null,
      });
    });
  });

  describe('revokeSession', () => {
    it('calls revokeSession with sessionToken', async () => {
      adminService.revokeSession.mockResolvedValue({ success: true } as never);

      await controller.revokeSession(baseSession, { sessionToken: 'tok-abc' });

      expect(adminService.revokeSession).toHaveBeenCalledWith(
        { sessionToken: 'tok-abc' }, 'admin', null,
      );
    });
  });

  describe('revokeAll', () => {
    it('calls revokeAllSessions with userId', async () => {
      adminService.revokeAllSessions.mockResolvedValue({ success: true } as never);

      await controller.revokeAll(baseSession, 'user-1');

      expect(adminService.revokeAllSessions).toHaveBeenCalledWith(
        { userId: 'user-1' }, 'admin', null,
      );
    });
  });

  describe('validatePagination — branch coverage', () => {
    it('throws on non-numeric limit — covers !isFinite branch', async () => {
      await expect(controller.list(baseSession, 'abc', '0')).rejects.toThrow(
        'limit must be a positive integer',
      );
    });

    it('throws on zero limit — covers parsedLimit <= 0 branch', async () => {
      await expect(controller.list(baseSession, '0', '0')).rejects.toThrow(
        'limit must be a positive integer',
      );
    });

    it('throws on negative offset — covers parsedOffset < 0 branch', async () => {
      await expect(controller.list(baseSession, '10', '-1')).rejects.toThrow(
        'offset must be a non-negative integer',
      );
    });

    it('throws on non-numeric offset — covers !isFinite offset branch', async () => {
  ...
  describe('validateRevokeSessionPayload — branch coverage', () => {
    it('throws when sessionToken is empty', async () => {
      await expect(controller.revokeSession(baseSession, { sessionToken: '' })).rejects.toThrow('sessionToken is required');
    });
  });
});
